  ...
  <div>
      <div>
          ...
          <div class="flex justify-between q-mb-sm">
            ...
            <q-input color="black" type="number" filled v-model="searchInput" label="Search by ID" style="width:200px;">
        ...
        <template #day="{ scope: { timestamp } }" >
          <template v-for="(entity, index) in entities" :key="index">
            <div class="flex flex-center q-mx-sm" v-if="entity.Detail.CreatedOn.substring(0, 10) === timestamp.date && filterEvents(entity.Detail, radioOption)">
              <q-chip :color="chipColour(entity.Detail)" text-color="white" size="90%" class="q-px-sm" :label="[entity.EntityId + ', ' + entity.WorkflowDataTypeName].toString()">
              </q-chip>
            </div>
          </template>
        </template>
